...
export default function LeadDetail({ params }: { params: { id: string } }) {
  ...
  if (loading) return <div className="p-8 text-gray-400">Loading lead details...</div>;
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="glass-card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              ...
              <div>
                <span className="block text-sm text-gray-500 mb-1">Business Name</span>
              ...
              <div className="md:col-span-2">
                <span className="block text-sm text-gray-500 mb-1">Website / Social</span>
              ...
              <div className="md:col-span-2">
                <span className="block text-sm text-gray-500 mb-1">Prefers Contact Via</span>
          ...
          <div className="glass-card p-6">
            ...
            <div className="space-y-6">
              <div>
                <span className="block text-sm text-gray-500 mb-1">Industry</span>
                <span className="font-medium">{lead.industry || 'Not Provided'}</span>
              </div>
              <div>
                <span className="block text-sm text-gray-500 mb-1">Interested Service</span>
                <span className="font-medium text-primary">{lead.interested_service || 'Not Provided'}</span>
              </div>
              <div>
                <span className="block text-sm text-gray-500 mb-1">Business Description</span>
                <p className="bg-black/50 p-4 rounded-lg border border-white/5">{lead.business_description || 'N/A'}</p>
              </div>
              <div>
                <span className="block text-sm text-gray-500 mb-1">Current Challenge</span>
                <p className="bg-black/50 p-4 rounded-lg border border-white/5">{lead.current_challenge}</p>
              </div>
              <div>
                <span className="block text-sm text-gray-500 mb-1">Goals</span>
                <p className="bg-black/50 p-4 rounded-lg border border-white/5">{lead.goals || 'N/A'}</p>
              </div>
              <div>
                <span className="block text-sm text-gray-500 mb-1">Why Good Fit?</span>
                <p className="bg-primary/5 p-4 rounded-lg border border-primary/20 text-white italic">"{lead.why_good_fit}"</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <span className="block text-sm text-gray-500 mb-1">Ready to Start</span>
                <span className="font-medium">{lead.ready_to_start}</span>
              </div>
              <div>
                <span className="block text-sm text-gray-500 mb-1">Worked with Agency?</span>
        ...
        <div className="w-full lg:w-80 space-y-6">
          <div className="glass-card p-6 sticky top-8">
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-sm text-gray-400 mb-1">Lead Status</label>
                <select 
                  value={status} 
                  onChange={(e) => setStatus(e.target.value)}
                  className="w-full bg-black border border-border rounded p-3 text-white focus:border-primary outline-none"
                >
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">Admin Notes (Hidden from Lead)</label>
                <textarea 
                  value={notes} 
